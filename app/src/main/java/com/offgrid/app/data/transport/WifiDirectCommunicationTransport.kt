...
class WifiDirectCommunicationTransport(
    ...
    data class Diagnostics(
        ...
        val deliveryAttemptRatePercent: Double
            get() = if (packetsSent == 0L) 0.0 else (packetsSent - packetsFailed).coerceAtLeast(0).toDouble() / packetsSent * 100.0
    }
    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            when (intent.action) {
                ...
                WifiP2pManager.WIFI_P2P_THIS_DEVICE_CHANGED_ACTION -> _status.value = "Wi-Fi P2P device identity updated"
            }
        }
    }
                ...
                override fun onFailure(reason: Int) {
                    ...
                    if (reason == WifiP2pManager.BUSY) {
                        ...
                        p2p.stopPeerDiscovery(ch, object : WifiP2pManager.ActionListener {
                            ...
                            override fun onFailure(reason: Int) { result.complete(false) }
                        })
                    } else {
    ...
    override suspend fun connectToDevice(node: Node): Result<Unit> = withContext(Dispatchers.IO) {
        ...
        if (existingSocket != null && peersBySocket[existingSocket] != null) return@withContext Result.success(Unit)
        ...
        runCatching {
            p2p.connect(ch, config, object : WifiP2pManager.ActionListener {
                ...
                override fun onFailure(reason: Int) {
                    ...
                    waiters.remove(address)?.complete(Result.failure(IllegalStateException("Wi-Fi P2P connect failed: ${reasonText(reason)}")))
                    _linkState.value = LinkState.OFFLINE
                }
            })
        }.onFailure { error ->
    ...
    override suspend fun sendMessage(message: Message): Result<Unit> = sendMutex.withLock {
        val requestedDestination = message.recipientNodeId ?: message.receiverId
        val discovered = _nodes.value.firstOrNull {
            it.id.equals(message.receiverId, ignoreCase = true) ||
                it.logicalId.equals(requestedDestination, ignoreCase = true)
        }

        // A chat can hold an older physical address after the HELLO handshake has supplied the
        // stable logical ID. Canonicalize the destination before routing so the packet reaches the
        // peer's stable identity rather than an Android P2P address.
        val directByAddress = message.receiverId
            .takeIf { it.isNotBlank() }
            ?.let { socketByDeviceAddress[it] }
            ?.let { peersBySocket[it] }

        val canonicalDestination = when {
            directByAddress != null -> directByAddress.logicalId
            discovered?.logicalId?.isNotBlank() == true -> discovered.logicalId
            else -> requestedDestination
        }

        var nextHop = router.nextHop(canonicalDestination, listOf(selfId))
        var connection = nextHop?.let { hop -> peersBySocket[socketByLogicalId[hop]] }

        // Direct physical mapping is the final fallback for a peer whose logical identity has not
        // yet propagated into the conversation state.
        if (connection == null) {
            connection = directByAddress
            if (connection != null) nextHop = connection.logicalId
        }

        // A directly connected peer is always a valid first hop even if route advertisements have
        // not arrived yet.
        if (connection == null) {
            val directLogical = discovered?.logicalId?.takeIf { it.isNotBlank() }
            connection = directLogical?.let { peersBySocket[socketByLogicalId[it]] }
            if (connection != null) nextHop = connection.logicalId
        }

        if (connection == null || nextHop == null) {
            return@withLock Result.failure(
                IllegalStateException("No active Wi-Fi P2P link to $canonicalDestination")
            )
        }

        val plaintext = messageJson(message, canonicalDestination).toString().toByteArray(Charsets.UTF_8)
        val packet = HopPacket(
            messageId = message.id,
            sourceNodeId = selfId,
            destinationNodeId = canonicalDestination,
        ...
        writePacket(connection, packet).fold(
            onSuccess = {
                _status.value = "Wi-Fi P2P • AES-256-GCM • sent to ${canonicalDestination.takeLast(4)}"
                Result.success(Unit)
            },
    ...
    override fun stop() {
        ...
        peersBySocket.clear(); pendingSockets.clear(); pendingWriters.clear()
        socketByLogicalId.clear(); socketByDeviceAddress.clear(); writerLocks.clear()
        seenPackets.clear(); processingPackets.clear()
        waiters.values.forEach { it.cancel() }; waiters.clear(); pendingConnectionAddress = null
        runCatching { server?.close() }; server = null
        if (registered) { runCatching { appContext.unregisterReceiver(receiver) }; registered = false }
        runCatching { manager?.removeGroup(channel, null) }
        _nodes.value = emptyList(); _linkState.value = LinkState.OFFLINE; _status.value = "Wi-Fi P2P stopped"
    ...
    private fun requestPeers() {
        ...
        if (!hasPermission()) { _status.value = "Wi-Fi P2P peer request blocked • permission missing"; return }
        if (!isLocationModeEnabled()) { _status.value = "Wi-Fi P2P peer request blocked • Location Mode is OFF"; return }
        runCatching { p2p.requestPeers(ch, peersListener) }.onFailure {
            _status.value = "Wi-Fi P2P requestPeers failed • ${it.message ?: "permission/state error"}"
        }
    }
    ...
    private fun startServer() {
        ...
        scope.launch {
            ...
            }.onFailure { _status.value = "Wi-Fi P2P server stopped: ${it.message ?: "socket error"}" }
        }
    }
    ...
    private fun connectToOwner(host: String) {
        scope.launch {
            runCatching {
                ...
                handleSocket(socket)
            }.onFailure { _status.value = "Wi-Fi P2P data channel failed: ${it.message ?: "socket error"}" }
        }
    }
    ...
    private fun handleSocket(socket: Socket) {
        scope.launch {
            ...
            runCatching { sendHello(writer) }.onFailure {
                removePeer(key); return@launch
            }
            try {
                while (!socket.isClosed) {
                    val line = reader.readLine() ?: break
                    if (line.toByteArray(Charsets.UTF_8).size > MAX_PACKET_BYTES) {
                        _status.value = "Wi-Fi P2P • oversized packet dropped"; continue
                    }
                    handleLine(key, line)
                }
            } catch (_: Throwable) {
                ...
                removePeer(key); runCatching { socket.close() }
            }
        }
    }
    ...
    private fun sendHello(writer: BufferedWriter) {
        writer.write(JSONObject().apply {
            put("kind", "OFFGRID_HELLO"); put("protocol", HELLO); put("nodeId", selfId); put("name", selfName)
        }.toString())
    ...
    private fun handleHello(socketKey: String, json: JSONObject) {
        ...
        val deviceAddress = pendingConnectionAddress ?: _nodes.value.firstOrNull { it.logicalId == logicalId }?.id
        ...
        pendingSockets.remove(socketKey); pendingWriters.remove(socketKey)
        ...
        sendRoutesToPeer(peer); announceRoutesToPeers()
    }
    ...
    private fun handleRoutes(socketKey: String, json: JSONObject) {
        ...
        for (index in 0 until routes.length()) {
            ...
            val advertisedPath = buildList {
                if (pathJson != null) for (i in 0 until pathJson.length()) {
                    val id = pathJson.optString(i); if (id.isNotBlank()) add(id)
                }
            }
            if (selfId in advertisedPath || destination in advertisedPath.dropLast(1)) continue
            router.learn(destination, peer.logicalId, (advertisedDistance + 1).coerceAtMost(8), advertisedPath)
        }
        refreshConnectedNodes()
    }

    private fun announceRoutesToPeers() { peersBySocket.values.forEach { sendRoutesToPeer(it) } }

    private fun sendRoutesToPeer(peer: PeerConnection) {
        val routes = JSONArray()
        router.routes().forEach { route ->
            routes.put(JSONObject().apply {
                put("destination", route.destinationNodeId); put("nextHop", route.nextHopNodeId); put("distance", route.distance); put("path", JSONArray(route.path))
            })
        }
        val control = JSONObject().apply { put("kind", ROUTE_ANNOUNCEMENT); put("routes", routes) }
    ...
    private suspend fun handleHopPacket(socketKey: String, bytes: ByteArray) {
        ...
        try {
            ...
            val plaintext = runCatching {
                HopEncryption.decrypt(packet.ciphertext, HopEncryption.linkKey(previousHop, selfId))
            }.getOrElse {
                _status.value = "Wi-Fi P2P • encrypted packet rejected"; return
            }
            updateDiagnostics(received = true)
            if (packet.destinationNodeId == selfId) {
                decodeAndEmit(plaintext, packet.hopCount + 1); seenPackets.add(packet.messageId)
                _status.value = "Wi-Fi P2P • delivered after ${packet.hopCount + 1} hop(s)"; return
            }
            if (packet.hopCount >= packet.maxHops || selfId in packet.path) {
                seenPackets.add(packet.messageId); _status.value = "Wi-Fi P2P • relay dropped: loop/hop limit"; return
            }
            val nextHop = router.nextHop(packet.destinationNodeId, packet.path + selfId) ?: packet.destinationNodeId
            val nextPeer = peersBySocket.values.firstOrNull { it.logicalId == nextHop }
            if (nextPeer == null) {
                _status.value = "Wi-Fi P2P • no route to ${packet.destinationNodeId.takeLast(4)}"; return
            }
            val forwarded = packet.copy(
                previousHopId = selfId,
                hopCount = packet.hopCount + 1,
                path = packet.path + selfId,
                ciphertext = HopEncryption.encrypt(plaintext, HopEncryption.linkKey(selfId, nextHop)),
            )
    ...
    private suspend fun writeLine(peer: PeerConnection, line: String, countPacket: Boolean = false): Result<Unit> {
        val lock = writerLocks.computeIfAbsent(peer.socketKey) { Mutex() }
        return lock.withLock {
            val started = System.nanoTime()
            runCatching {
                peer.writer.write(line); peer.writer.newLine(); peer.writer.flush()
                if (countPacket) updateDiagnostics(sent = true, latencyMs = (System.nanoTime() - started) / 1_000_000)
            }.fold(
                onSuccess = { Result.success(Unit) },
                onFailure = { error ->
                    if (countPacket) updateDiagnostics(failed = true)
                    removePeer(peer.socketKey); Result.failure(error)
                },
            )
        }
    }
    ...
    private fun removePeer(socketKey: String) {
        ...
        peer.deviceAddress?.let { if (socketByDeviceAddress[it] == socketKey) socketByDeviceAddress.remove(it) }
        writerLocks.remove(socketKey); router.clearPeer(peer.logicalId); updateDiagnostics(disconnect = true); refreshConnectedNodes(); announceRoutesToPeers()
    }
    ...
    private fun messageJson(message: Message, destination: String) = JSONObject().apply {
        put("kind", "OFFGRID_MESSAGE"); put("id", message.id); put("conversationId", message.conversationId)
        put("senderId", message.senderId); put("receiverId", message.receiverId); put("recipientNodeId", destination)
        put("content", message.content); put("timestamp", message.timestamp); put("status", message.status.name); put("type", message.type.name)
    }
    ...
    private fun hasPermission(): Boolean {
        ...
        val accessWifi = ContextCompat.checkSelfPermission(appContext, Manifest.permission.ACCESS_WIFI_STATE) == PackageManager.PERMISSION_GRANTED
        val changeWifi = ContextCompat.checkSelfPermission(appContext, Manifest.permission.CHANGE_WIFI_STATE) == PackageManager.PERMISSION_GRANTED
    ...
    private fun isLocationModeEnabled(): Boolean = runCatching {
        Settings.Secure.getInt(appContext.contentResolver, Settings.Secure.LOCATION_MODE) != Settings.Secure.LOCATION_MODE_OFF
    }.getOrDefault(false)

    private fun refreshDiagnostics(p2pEnabled: Boolean = _diagnostics.value.p2pEnabled, peerCount: Int = _nodes.value.size) {
